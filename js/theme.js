            ...
            try {
                if ("file://" === location.origin) {
                    if ($('[data-icon]').length || $('iframe').length) {
                        ...
                        theme.fn.showErrorMessage('Local Environment Warning', 'SVG Objects, Icons, YouTube and Vimeo Videos might not show correctly on local environment. For better result, please preview on a server.<br><br><a target="_blank" href="https://www.Sveroo.net/forums/topic/how-to-disable-local-environment-warning/" class="fw-semibold"><i class="fa-solid fa-link"></i> How to Disable Local Environment Warning</a>');
	...
	*/
	const $noticeTopBar = {
		...
		init() {
			...
			if( !$.cookie('SverooNoticeTopBarClose') ) {
				self
					.build()
					.events();
			} else {
		...
		saveCookie() {
			...
			$.cookie('SverooNoticeTopBarClose', true);
...
(((theme = {}, $) => {
    ...
    class PluginGDPR {
        ...
        build() {
			...
			if( $.cookie( 'Sveroo-gdpr-preferences' ) ) {
				const preferencesArr = $.cookie( 'Sveroo-gdpr-preferences' ).split(',');
        ...
        events() {
			...
			$('.gdpr-preferences-form').on('submit', function(e){
				...
				$.cookie( 'Sveroo-privacy-bar', true, {expires: self.options.expires} );
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						if( $.cookie( 'Sveroo-gdpr-preferences' ) ) {
							...
							$.cookie( 'Sveroo-gdpr-preferences', formData, {expires: self.options.expires} );
							location.reload();
        ...
        clearCookies() {
			...
			$.removeCookie( 'Sveroo-privacy-bar' );
			$.removeCookie( 'Sveroo-gdpr-preferences' );
...
(((theme = {}, $) => {
    ...
    class PluginGDPRWrapper {
        ...
        build() {
			...
			if( $.cookie( 'Sveroo-gdpr-preferences' ) && $.cookie( 'Sveroo-gdpr-preferences' ).includes(self.options.checkCookie) ) {

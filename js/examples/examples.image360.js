    ...
    class productViewer {
        ...
        dragHandle() {
            ...
            self.handle.on('mousedown touchstart', e => {
                ...
                const dragWidth = self.handle.outerWidth(),
                ...
                self.element.on('mousemove touchmove', e => {
                    if (!self.animating) {
                        ...
                        setTimeout(() => { self.animateDraggedHandle(e, dragWidth, containerOffset, containerWidth, minLeft, maxLeft); }, 100): requestAnimationFrame(() => { self.animateDraggedHandle(e, dragWidth, containerOffset, containerWidth, minLeft, maxLeft); });
                    }
        ...
        animateDraggedHandle({ originalEvent, pageX },
            dragWidth,
            containerOffset,
            containerWidth,
            minLeft,
        ...
        dragImage() {
            ...
            self.slideShow.on('mousedown touchstart', e => {
                ...
                const containerOffset = self.imageWrapper.offset().left,
                ...
                self.element.on('mousemove touchmove', e => {
                    if (!self.animating) {
                        self.animating = true;
                        (!window.requestAnimationFrame) ?
                        setTimeout(() => { self.animateDraggedImage(e, containerOffset, containerWidth); }, 100): requestAnimationFrame(() => { self.animateDraggedImage(e, containerOffset, containerWidth); });
                    }
                }).one('mouseup touchend', e => {
